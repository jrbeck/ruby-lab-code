...
class NeuralNet
  ...
  def initialize_new_weights()
    # weight scalar for the first weight matrix. It is indirectly proportional to the input size.
    # It has to be, otherwise you will blow up your hidden layer. If you do not put in the deflator.
    # The first couple of iterations will work on large sums in the hidden layer. Large sums are
    # bad in neural networks.
    @init_factor_1 = 0.01 / ((@input_size + 1) ** (0.5))
    @init_factor_2 = 0.01 / (@hidden_nodes ** (0.5))

    # some basic matrix algebra, create a matrix with
    @w1 = (NMatrix.random([@input_size + 1, @hidden_nodes]) - (NMatrix.ones([@input_size + 1, @hidden_nodes]) / 2)) * @init_factor_1
